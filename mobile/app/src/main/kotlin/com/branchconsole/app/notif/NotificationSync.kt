...
private fun TickInputEntity.toEngineTick() = Tick(composite, distinctAxes, anyCrit, anyExtreme)
 ...
 * [AssetConfigSource]만 재사용한다(읽기 참조는 허용 — `com.branchconsole.app.preview` 패키지가
 * 이미 `tick` 패키지의 `ConfirmTickContext`·`ConfirmTickConfig` 등을 그대로 소비하는 것과 동일한
 * 선례).
 *
 * 커서는 평문 `SharedPreferences`(비밀 아님 — 그냥 "마지막으로 확인한 지점")로 유지해 중복·과거
 * 재알림을 막는다. WorkManager 배선은 [NotificationSyncWorker]가 담당하고, 이 클래스는
 * WorkManager를 전혀 모른다(순수 조립 + 부수효과 격리 — [notify] 콜백을 테스트가 가로챈다).
 */
internal class NotificationSync(
    ...
    private val notify: (channelId: String, notificationId: Int, title: String, text: String) -> Unit =
        { channelId, notificationId, title, text ->
            if (NotificationGate.isEnabled(context)) NotificationSender.send(context, channelId, notificationId, title, text)
        },
) {
    ...
    private suspend fun checkPhaseTransition() {
        ...
        val timeline = StateMachine.run(rows.map { it.toEngineTick() }, profile, statemachineConfig)

        val lastNotifiedDate = prefs.getString(KEY_LAST_PHASE_DATE, null)
        val newIndices = rows.indices.filter { i -> lastNotifiedDate == null || rows[i].tradingDate > lastNotifiedDate }
    ...
    @Suppress(
        "ReturnCount",
    ) // guard-clause style (empty history / nothing new / already budgeted today) -- same judgment as checkPhaseTransition above.
