...
## 14. Stage B 절차 기록(사후, qa-verifier 반려분 대응)

Stage A(본 문서)는 확정본이며 아래는 그리드·게이트·선정 규칙을 재설계하지 않는다 —
Stage B(`backtest/run_sweep.py` 구현·실행) 과정에서 발생한, 이 문서에 사전 기재되지
않았던 두 가지 절차·구현 판단을 사실 그대로 기록한다. 선정 결과·configs 반영값은
이 기록으로 인해 변경되지 않는다(qa-verifier가 독립 재현으로 선정 결과 자체는 건전함을
이미 확인).

### 14.1 기준선 `backtest/results/metrics.json` 재생성 (사전 승인 없음)

Stage B 실행 중 `backtest/results/metrics.json`(0.2.0 기준 9창×2프로파일 리플레이 산출)을
0.3.0-rc configs 반영 **후** 재생성했다. 이 재생성은 브리프에 명시된 완료 기준이 아니었고
사전 승인 없이 이뤄졌다 — MT0-05①(O-3 해소) 선례("registry_version 표기 불일치 재생성")를
유추 적용한 재량 판단이었다. 사실관계:

- 재생성 전(0.2.0 기준선) 파일은 git 추적 대상이 아니다(`backtest/results/`는
  `.gitignore` 대상) — 즉 이 재생성으로 커밋된 이력이 유실되지는 않았다.
- qa-verifier가 0.3.0-rc configs를 되돌린 상태에서 `run_replay.py --profile both
  --window all`을 재실행해 0.2.0 기준선 payload가 원래 값과 일치함을 재현으로 확인했다
  (재현성 검증 완료 — 데이터 유실 없음).
- 이후 서브태스크가 이 파일을 "0.2.0 기준선"으로 기대하고 참조한다면 어긋난다 — 필요 시
  `git stash`/별도 브랜치로 0.2.0 스냅샷을 보존하거나, 재생성 여부를 브리프에 명시하는
  절차를 다음 스윕부터 적용한다.

### 14.2 S1~S3 단계의 "완화 게이트" — 설계 문서에 없던 구현 재량

`backtest/run_sweep.py`의 S1(usdkrw_thresholds)·S2(stale daily_us)·S3(교차검증) 세 단계는
③(mobile_daily 프로파일)을 0.2.0 기준값에 고정한 채 도는데, 그 기준값 자체가 이미 §6
mobile 플래핑(양성≤6)을 위반한다(양성창 최대 전이 7, 본 문서 §2 표). 즉 ③이 실제로
바뀌는 S4 이전에는 완전판 게이트(플래핑 포함)가 전 후보에서 상시 FAIL이라 "게이트 통과
조합 중 rank"라는 §6.2 문언을 이 세 단계에 그대로 적용하면 대상 집합이 공집합이 된다.
이에 Stage B 구현은 **완화 게이트**(detection∧leadtime∧false_positive, flapping 제외)를
S1/S2/S3의 내부 top_k·불일치 판정에 한해 적용하고, **S4(mobile_daily_profile) 최종
선정에는 flapping을 포함한 완전판 게이트를 그대로 적용**했다(`backtest/run_sweep.py`
모듈 docstring·`reduced_gate_pass()` 참조).

이 처리는 `sweep.yaml`·본 설계 문서 어디에도 명문화되지 않은 **Stage B 구현 재량**이었다
— qa-verifier가 반려 사유로 지적한 대로 사전 기록 없이 이뤄졌다. `sweep.yaml`은 Stage A
확정본이므로 이 기록을 반영해 수정하지 않는다(본 절이 사후 기록의 전부다).

**qa-verifier 실증(독립 재현) — 이번 실행 결과에 수학적으로 무영향임을 확인**:

- S1(usdkrw_thresholds, golden 통과 24건) · S2(stale daily_us, golden 통과 3건) 모두,
  완전판 게이트(플래핑 포함)를 적용하면 **100% 탈락(0/24, 0/3)**한다 — 즉 완전판 게이트를
  1순위 필터로 썼어도 통과 집합이 공집합이 되어 `_rank_golden_pool()`의 폴백 경로(게이트
  없이 golden-pass 전체를 rank)로 똑같이 떨어진다.
- 실제로 완화 게이트로 필터링한 뒤 rank한 순서와, 폴백 경로(golden-pass 전체를 곧바로
  rank)로 rank한 순서를 비교하면 **후보 집합·순서가 완전히 동일**하다(S1 24개 전 순서
  일치, S2 3개 전 순서 일치 — 독립 재현으로 확인, 검증 스크립트는 `sweep_result.json`의
  `stages.S1/S2.entries`와 `run_sweep.rank_sort_key`로 재현 가능).
- 결론: 이 구현 재량은 **이번 실행에서 결과를 바꾸지 않았다** — S1/S2/S3가 완화 게이트
  대신 완전판 게이트를 썼더라도(공집합 → 폴백) 선정된 0.3.0-rc 값은 동일했을 것이다.
  다만 이는 "이번 그리드·이번 기준선"에서 우연히 성립하는 사실이지 일반 정리가 아니므로,
  다음 스윕(BT-04 이후 재보정 등)에서 ③ 기준값이 바뀌어 완전판 게이트 통과 후보가
  생기면 이 무영향 관계가 깨질 수 있다 — 재사용 시 재검증 필요.
