...
> **`upgrade.RED`에는 `or_any_*` 키가 없어 `exit_RED`는 어떤 경우에도 차단되지
> 않는다** — pairing은 upgrade 규칙에 실제로 이스케이프 키가 선언된 레벨에만 적용되고,
> RED는 현재도 향후에도 그런 선언이 없는 한 이 결정의 영향을 받지 않는다.
> 근거·실측: docs/journal/2026-08-03_MT0-07_pairing_design.md §1(aaa-critic 2라운드 PASS).

## D-26. 이스케이프-이탈 짝지음 (2026-08-03, GATE_GM0 안건 5 — 사용자 승인, 방향 채택)

`or_*` 계열 진입 이스케이프와 composite 단독 이탈 조건의 조합은 "이스케이프 참 + 저 composite"
구간에서 진입·이탈이 동시 영구 충족되어 한계진동을 만든다 — 데이터 무관 설정 성질(BT-03 §9.2:
`or_any_crit ⊕ exit_AMBER` 상수 입력 재현, server 15/mobile 49전이), BT-04에서 동형 쌍
(`or_any_extreme ⊕ exit_ORANGE`)으로 병리의 일반성이 독립 실증됨(w2026 mobile 전이 6→9).

**결정(수정 방향 A 채택)**: 이스케이프 근거가 지속되는 동안 해당 레벨의 이탈 조건을 미충족으로
취급한다 — 히스테리시스를 진입 근거와 짝지음. 적용 범위는 **`or_*` 이스케이프 전 계열 공통**
(프로덕션 `or_any_crit`, 실험 `or_any_extreme` 및 향후 신설분).

**제약(구현 선행 조건, MT0-07)**: ① 골든 무회귀(D-08 2케이스×2프로파일) 재확인 필수 — AMBER
진입·이탈 경로를 건드리므로 `golden_mobile.yaml` 동결 타임라인 영향을 실측으로 확정 ② 이스케이프
장기 지속 시 국면 영구 고착 위험(BT-03 §13 유의점)에 대한 상한 규율 필요 여부를 설계 단계에서
실측 근거로 판정 ③ 정확한 실행 의미론(스트릭·dwell·cooldown과의 상호작용)은 D-25 부기 형식으로
MT0-07 설계 저널에 확정 후 본 결정에 역참조를 남긴다 — **이행 완료(2026-08-03)**:
확정 의미론은 위 D-25 말미 부기(레벨-로컬·reset·RED 예외), 원 설계·실측은
docs/journal/2026-08-03_MT0-07_pairing_design.md §1·§4·§5 ④ Kotlin 패리티(BT-05) 형상 변경이므로
M1 착수 전 완결한다.
