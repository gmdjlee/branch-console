...
2. **ECOS·CDS 이중 차단 → 프리뷰 상시 억제(A-1)** — ECOS 미발급(브리프 §4 "선택") +
   CDS 미수집 확정(M-20 (b))이 겹치면 프리뷰 `rawCoverage`가 0.80 미만으로 떨어져
   (ECOS 차단 시 0.792) 프리뷰가 상시 억제 상태가 될 수 있다. S-3에서 `suppressed=true`가
   3회 모두 뜨면 이 조합이 원인일 가능성이 높다 — ECOS 키 입력 여부를 기록.
...
4. **credit 축 발화 표면 축소** — CDS 미수집(M-20 (b))으로 credit 축은 사실상 회사채
   스프레드(ECOS) 단일 계열에 의존한다. ECOS 미발급 시 credit 축이 사실상 항상 결측임을
   기록(정상 동작 — 버그 아님).
