...
| **② MT1-04d ECOS 어댑터** | 사용자 결정(2026-08-07): **키 발급 후 M1 내 구현**. 키 도착 → 00b 재개(item_code 실측·VERIFY 교체) → 04d 구현·2단 판정 | 키 대기 |
...
| A-1 프리뷰 커버리지 상한 | ECOS 차단 시 0.792 < 0.80(상시 억제 = 정상 동작 선언·구현 반영) — **키 발급 시 0.847로 해소**. 미발급 장기화 시 D-23 개정 상신 | ②와 연동 |
