...
- 상태: **초안 — 통과 선언 불가.** 게이트 조건 4건 미충족(§6): ① confirm_time_kst 실측·기입(MT1-00g)
  ② MT1-04d(ECOS 키 대기) ③ 실기기 1일 스모크 미실행 ④ 사용자 승인. 4칸 충족 시 본 문서를 갱신해 상신한다.
...
대기. 구현 잔여는 **MT1-04d 1건**(ECOS 키 발급 대기 — 사용자 결정: M1 내 구현)이며 그 외 기본 8·신설 12 서브태스크는
전건 인도·흡수 정산 완료(aaa 전수 대조).
...
| MT1-00 실측 선행(신설) | 00a 야후·Stooq·FRED(63d4cf0·87e059c·38a8ab7 — Stooq PoW 차단 확정·FRED 폴백 매핑·HY OAS 3년 롤링) / 00b **BLOCKED**(ECOS 키 — 30195ef) / 00c kotlin_krx(3012100·2a32f84 — TRDVAL 오정렬 적발·정정) / 00d CDS (b) 미수집(a7b0322) / 00e 툴체인(cdebe47+3) / 00f SQLite 플랜(4039bb6) / 00g 계기+1일차 폴링(523065d·d569cab·d075daa — **3거래일 미완**) | 부분(00b·00g) |
...
| MT1-04 collectors | 04a·b(47fd335·c326428 — 00a 계약·오류 6분류·RetryPolicy SSOT 로더) / 04c(eeee805 — K-03 SSOT 스로틀·K-19·TRDVAL 계약 증인) / **04d 잔여(키 대기)** / 04e M2 이연(사용자) / 04f (b) 미수집(사용자) / 04g 웜업 백필(564d631) / 04h PIT 대조(2611123) | 부분(04d) |
...
| 미결 | 처리 경로 | 상태 |
|---|---|---|
| **① confirm_time_kst 실측·기입** | MT1-00g — 1일차(08-07) 6슬롯 완료(16:00에 3/4 확정 관측). **08-10·08-11 폴링 → 사전등록 규칙 판정 → statemachine.yaml 기입**. 미기입 시 스케줄·스모크 S-2 실행 불가(명시 실패 설계) | 진행 중 |
| **② MT1-04d ECOS 어댑터** | **인도 완료(2026-08-08)**: 00b 재개 — 실측이 SSOT 오기(721Y001→일별 정본 **817Y002**)를 적발·정정, item_codes 확정, **K-04 종결**(c7f9fab). 04d 수집기 구현(0eb6b98 — 수집 계층, 파이프라인 3경로 편입). **지표(krx_credit_spread_delta) 활성화는 C1 소관 별도 지위**: Python 정본 빌더 부재 + 활성화 시 composite 분모 82.5→88.5로 전 틱 ×0.9322 — 골든 재산출 산술 확정(F-04 동형) | M-1 정정 반영 중 |
