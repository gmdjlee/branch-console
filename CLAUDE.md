...
- **구현 노동은 전부 서브에이전트(Worker)에게 위임한다.** Task 도구로 위임한다. 모델 배정은 D-20 §20.2 고정이다(§1 참조): 계획·비평=claude-opus-5, 구현·검증=claude-sonnet-5. Worker를 상위 모델로 올리려면 그 판단 근거를 브리프에 남긴다.
