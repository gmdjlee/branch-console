...
- W0 실측 선행 (판정: qa R1 PASS→aaa R1 FAIL(D-1~5·A-1~6)→수정→qa R2 PASS→**aaa R2 PASS** — REVIEW_M1)
  ...
  - [ ] MT1-00e 툴체인 호환 매트릭스 — **구현 완료·2단 판정 중**(cdebe47·d6c39c4): AGP 8.13.2+
    Gradle 8.13+Kotlin 2.1.0+JBR 21+compileSdk 36, 추가 다운로드 0. detekt 1.23.8 호환 공백·
    jvmToolchain 함정·Konsist 파서 동결 리스크 실증. 카탈로그 초안 §6
  - [ ] MT1-00f SQLite 인덱스 플랜 — **구현 완료·2단 판정 중**(4039bb6): 채택 쿼리 전부 인덱스
    탐색·풀스캔 0, 로컬 3.50.4≠Android 3.22 한계 명시(sqlite_version 로깅 제안 → MT1-03)
  - [ ] MT1-02a 계약 스냅샷(Python 측) — **구현 완료·2단 판정 중**(3b5ddf0): 스냅샷 12파일+
    생성기(결정론)+동결 테스트 20건, 205 green·커버리지 99%. Kotlin 미러는 01a 후
